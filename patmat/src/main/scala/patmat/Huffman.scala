...
trait Huffman extends HuffmanInterface {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case head :: neck :: tail =>
      (Fork(head, neck, chars(head) ::: chars(neck), weight(head) + weight(neck)) :: tail)
        .sortWith((prev, cur) => weight(prev) < weight(cur))
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(innerTree: CodeTree, innerBits: List[Bit]): List[Char] =
      innerTree match {
        case Leaf(c, _) => c :: traverse(tree, innerBits)
        case Fork(left, right, _, _) => innerBits match {
          case Nil => Nil
          case head :: tail => head match {
            case 0 => traverse(left, tail)
            case 1 => traverse(right, tail)
          }
        }
      }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table match {
      case Nil => throw new NoSuchElementException("Not found")
      case (c, lst) :: tail => if (c == char) lst else codeBits(tail)(char)
    }
