...
class ListsSuite {
  ...
  @Test def `sum of one number (10pts)`: Unit = {
    assert(sum(List(45)) == 45)
  }

  @Test def `sum of a zero numbers (10pts)`: Unit = {
    assert(sum(List()) == 0)
  }
  ...
  @Test def `max of one number (10pts)`: Unit = {
    assert(max(List(3)) == 3)
  }

  @Test def `max of 0 number (10pts)`: Unit = {
    assert(max(List()) == 0)
  }
